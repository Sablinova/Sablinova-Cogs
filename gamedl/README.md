...
A Red-DiscordBot cog to quickly search and extract direct PC game download links, versions, file sizes, and Steam portrait cover art directly into Discord.
...
- **Unified Meta-Search**: Concurrently searches across multiple direct download sources, seamlessly merging mirrors so you get all available links in one place.
...
- `[p]gamedl <game name or AppID>` - Unified search across all supported sources with merged mirrors (alias: `[p]gdl`).
