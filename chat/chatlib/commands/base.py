...
class ChatBase(BaseCog):
    ...
    async def get_gemini_token(self):
        self.gemini_token = os.environ.get("GEMINI_API_KEY")
        if self.gemini_token is None:
            self.gemini_settings = await self.bot_instance.get_shared_api_tokens(
                "gemini"
            )
            self.gemini_token = self.gemini_settings.get("key", None)
        return self.gemini_token
